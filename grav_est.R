...
#### read in data 
load( "../Data/WorkData/impuData15.Rdata" )
...
form <- formula( newarrival ~ index0asylum + log( dist ) + contig + comlang_off +
                 ...
                 log( pop_o ) + CL_o + CL_d + PR_o + PR_d + asinh( CPI_d ) + asinh( CPI_o ) + 
                 dead_d + dead_log_d + dead_o + dead_log_o +
                 Nyear_conf_d + Nyear_log_d + Nyear_conf_o + Nyear_log_o | iso_o + iso_d )

### loop through five imputed data sets 
for( i in 1:5 ){
   ## alpaca 
   # poisson 
   assign( paste0( "lama1_", i ), feglm( formula = form, data = impu15[[i]], family = poisson( )))

   # negative binomial
   assign( paste0( "lama2_", i ), feglm.nb( formula = form, data = impu15[[i]] ))

   ## FENmlm
   # poisson
   assign( paste0( "fennel1_", i ), femlm( form, data = impu15[[i]], family = "poisson", na.rm = TRUE ))

   # negative binomial
   assign( paste0( "fennel2_", i ), femlm( form, data = impu15[[i]], family = "negbin", na.rm = TRUE ))
}
